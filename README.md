...
* **`dataset.py` & `data_loader.py`**: Contains the `MultimodalManifestDataset` and custom `collate_fn` for variable-length padding, attention masking, and dynamic foundation model feature extraction.
...
* **`datamodule.py`**: The PyTorch Lightning DataModule wrapper for streamlined train, validation, and test split orchestration.
...
3. Ensure you have the raw CMU-MOSEI dataset downloaded and accessible.

## Usage
To train the model from scratch using the default configuration, run the main execution script:

```bash
python train.py --data_dir /path/to/cmu_mosei --batch_size 16 --learning_rate 1e-4 --max_epochs 50
```

To fine-tune the foundation model backbones (ensure a low learning rate):

```bash
python train.py --data_dir /path/to/cmu_mosei --unfreeze_backbones --learning_rate 1e-5
```
